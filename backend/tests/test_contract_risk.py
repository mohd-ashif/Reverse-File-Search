...
def test_analyze_404_when_file_missing(auth_client: TestClient, monkeypatch) -> None:
    monkeypatch.setattr("app.services.contract_risk_service.get_groq_client", lambda: FakeGroqClient())
    response = auth_client.post("/api/v1/files/999999/contract-risks")
    assert response.status_code == 404


def test_analyze_503_when_not_configured(auth_client: TestClient, db_session: Session, tmp_path, monkeypatch) -> None:
    ...
    response = auth_client.post(f"/api/v1/files/{file_record.id}/contract-risks")
    assert response.status_code == 503


def test_analyze_422_when_file_has_no_text(auth_client: TestClient, db_session: Session, tmp_path, monkeypatch) -> None:
...
def test_analyze_returns_all_categories_in_order(auth_client: TestClient, db_session: Session, tmp_path, monkeypatch) -> None:
...
def test_analyze_fills_missing_categories_defensively(auth_client: TestClient, db_session: Session, tmp_path, monkeypatch) -> None:
